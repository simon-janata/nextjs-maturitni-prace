...
export async function GET(req: Request, res: Response) {
	try {
		const job = new CronJob("0 0 0 * * *", async function() {
			try {
				const schoolYears = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/cs/api/schoolYears`);

				if (schoolYears.data && schoolYears.data.length > 0) {
					for (const schoolYear of schoolYears.data) {
						...
						await axios.delete(`${process.env.NEXT_PUBLIC_API_URL}/cs/api/schoolYears/${schoolYear.year}`);
					}
				}
			} catch (err) {
